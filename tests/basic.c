...
CCINT32 test_basic_failures(void) { return failed; }
...
static void _expect_(const char *name, const char *input, CCINT32 expected, CCUINT32 case_code)
{
    ...
    else
    {
        ...
        color_print(
            ...
            CC_TEXT_COLOR(CC_DEFAULT, CC_DEFAULT) " %s input=\"%s\", expected %d, got %d\n",
...
static void test_cc_atoi(void)
{
    ...
    _expect_("overflow 10^19", "10000000000000000000", 2147483647, case_code++);
    _expect_("underflow -10^19", "-10000000000000000000", -2147483648, case_code++);
    _expect_("long leading zeros", "0000000000000000000000000001", 1, case_code++);
    _expect_("long leading zeros negative", "-0000000000000000000000000005", -5, case_code++);
}
